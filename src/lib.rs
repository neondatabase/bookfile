...
const BOOKWRITER_V1_MAGIC: u32 = 0xFF33_0001;
...
impl<'a, W> Chapter<'a, W>
where
    W: Write,
{
    fn new(book: &'a mut BookAuthor<W>, id: u64, offset: usize) -> Self {
        Chapter {
...
impl<'a, W> Write for Chapter<'a, W>
where
    W: Write,
{
...
impl<W: Write> BookAuthor<W> {
    pub fn new(writer: W, user_magic: u32) -> Result<Self> {
        let mut this = BookAuthor {
            writer,
            current_offset: 0,
            header: FileHeader {
                bookwriter_magic: BOOKWRITER_V1_MAGIC,
                user_magic,
            },
            toc_entries: vec![],
        };
    ...
    pub fn new_chapter(&mut self, id: u64) -> Chapter<'_, W> {
        Chapter::new(self, id, self.current_offset)
    }
...
mod tests {
    ...
    fn empty_book() {
        ...
        let book = BookAuthor::new(buffer, magic).unwrap();
        book.close().unwrap();
    }
    ...
    fn simple_book() {
        let magic = 0x1234;
        let buffer = Cursor::new(Vec::<u8>::new());
        let mut book = BookAuthor::new(buffer, magic).unwrap();
